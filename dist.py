...
Covariance = jnp.ndarray  # full covariance or diagonal
Precision = jnp.ndarray  # full covariance or diagonal
...
class SingleEL(object):
    # Weight of EL of the i^th data point
    def __init__(self, h: Callable, data: Any, i: int, use_llog=True) -> None:
        self._h = h
        self._data = data
        self._i = i
        self._use_llog = use_llog

    def raw_logpdf(self, x: jnp.ndarray) -> jnp.ndarray:
        return self.compute_raw_logpdf(
            x=x, h=self._h, data=self._data, i=self._i, use_llog=self._use_llog
        )

    @staticmethod
    def compute_raw_logpdf(
        x: jnp.ndarray,
        h: jnp.ndarray,
        data: jnp.ndarray,
        i: jnp.ndarray,
        ...
        """Compute log of a single EL weight, up to some constants that do not
        depend on the `x`.
        """
        # lbd = partial(emplik.calc_lambda, h, data)
        lbd = emplik.calc_lambda(h, data, x)
        dp = jax.tree_map(lambda x: x[None, i], data)
        # return emplik.calc_unscaled_log_wi(lbd[0], h, dp, x)
        log_wi = emplik.calc_log_w(lbd[0], h, dp, x, use_llog=use_llog)
        return log_wi.squeeze()

    def tree_flatten(self) -> tuple:
        children = (self._data, self._i)  # arrays / dynamic values
        aux_data = {"h": self._h}  # static values
        return (children, aux_data)

    @classmethod
    def tree_unflatten(cls, aux_data, children) -> "SingleEL":
        data, i = children
        return cls(h=aux_data["h"], data=data, i=i)
...
class Gaussian(object):
    ...
    def nll(self, x: jnp.ndarray) -> jnp.ndarray:
        return -1.0 * self.compute_raw_logpdf(x=x, eta=self.eta, Lambda=self.Lambda)

    def raw_logpdf(self, x: jnp.ndarray) -> jnp.ndarray:
        # unnormalized log density
        return self.compute_raw_logpdf(x=x, eta=self.eta, Lambda=self.Lambda)
    ...
    def sample(self, prng_key: jax.random.PRNGKey) -> jnp.ndarray:
        raise NotImplementedError

    def tree_flatten(self) -> tuple:
        children = (self._mu, self._eta, self._Sigma, self._Lambda)
        aux_data = None
        return (children, aux_data)

    @classmethod
    def tree_unflatten(cls, aux_data, children) -> "Gaussian":
        return cls(*children)
...
class FullGaussian(Gaussian):
    ...
    @classmethod
    def symmetrify_cov(cls, q: "FullGaussian") -> "FullGaussian":
        if q._Lambda is not None:
            return cls(eta=q._eta, Lambda=0.5 * (q._Lambda + q._Lambda.T))
        else:
            return cls(mu=q._mu, Lambda=0.5 * (q._Sigma + q._Sigma.T))
    ...
    def compute_raw_logpdf(
        ...
        return eta.T @ x - 0.5 * x.T @ Lambda @ x
    ...
    def sample(self, prng_key: jax.random.PRNGKey) -> jnp.ndarray:
        return jax.random.multivariate_normal(prng_key, self.mu, self.Sigma)


@jax.tree_util.register_pytree_node_class
class DiagonalGaussian(FullGaussian):
...
def test_gaussian() -> None:
    ...
    for _ in range(num_gaussians):
        key, subkey = jax.random.split(key)
        mu = jax.random.normal(key=subkey, shape=[num_units])
        key, subkey = jax.random.split(key)
        Sigma = jax.random.normal(key=subkey, shape=[num_units, num_units])
        q1s.append(FullGaussian(mu=mu, Sigma=Sigma))

    for _ in range(num_gaussians):
        key, subkey = jax.random.split(key)
        eta = jax.random.normal(key=subkey, shape=[num_units])
        key, subkey = jax.random.split(key)
        Lambda = jax.random.normal(key=subkey, shape=[num_units, num_units])
        q2s.append(FullGaussian(eta=eta, Lambda=Lambda))
    ...
    for qs in [q1s, q2s, q1s + q2s]:
        qs_combined = FullGaussian.from_products(qs)
        _Sigma = jnp.linalg.inv(
            jnp.sum(jnp.stack([jnp.linalg.inv(q.Sigma) for q in qs], axis=0), axis=0)
        )
        _Lambda = jnp.sum(jnp.stack([q.Lambda for q in qs], axis=0), axis=0)
        _mu = jnp.matmul(
            _Sigma,
            jnp.sum(
                jnp.stack(
                    [jnp.matmul(jnp.linalg.inv(q.Sigma), q.mu) for q in qs], axis=0
                ),
                axis=0,
            ),
        )
        _eta = jnp.sum(jnp.stack([q.eta for q in qs], axis=0), axis=0)
    ...
    for qs in [q1s, q1s[::-1], q2s, q2s[::-1], q1s + q2s, (q1s + q2s)[::-1]]:
        _q1 = qs[0]
        _q2 = qs[-1]
        qs_divided = FullGaussian.from_quotient(_q1, _q2)
        _Sigma = jnp.linalg.inv(jnp.linalg.inv(_q1.Sigma) - jnp.linalg.inv(_q2.Sigma))
...
def test_diagonal_gaussian() -> None:
    ...
    def get_random_x():
        return jnp.array(np.random.normal(size=[d]))

    def get_random_eta():
        return jnp.array(np.random.normal(size=[d]))

    def get_random_Lambda():
        return jnp.array(np.random.normal(size=[d, d]))

    def _check_equality(qd: FullGaussian, q: FullGaussian) -> None:
        print(
            (qd.eta == q.eta).all(),
            "\t",
        ...
        print(
            (jnp.diagflat(qd.Lambda) == q.Lambda).all(),
            "\t",
    ...
    for _ in range(3):
        ...
        qd1 = DiagonalGaussian(
            eta=get_random_eta(), Lambda=jnp.diagonal(get_random_Lambda())
        )
        ...
        q1 = FullGaussian(eta=qd1.eta, Lambda=jnp.diagflat(qd1.Lambda))
        q2 = FullGaussian(eta=qd2.eta, Lambda=jnp.diagflat(qd2.Lambda))
        q3 = FullGaussian(eta=qd3.eta, Lambda=jnp.diagflat(qd3.Lambda))

        print(jnp.allclose(qd1.nll(x), q1.nll(x)), "\t", qd1.nll(x), "\t", q1.nll(x))
        print(jnp.allclose(qd2.nll(x), q2.nll(x)), "\t", qd2.nll(x), "\t", q2.nll(x))
        print(jnp.allclose(qd3.nll(x), q3.nll(x)), "\t", qd3.nll(x), "\t", q3.nll(x))

        _check_equality(
            DiagonalGaussian.from_products([qd1, qd2, qd3]),
            FullGaussian.from_products([q1, q2, q3]),
        )

        _check_equality(
            DiagonalGaussian.from_quotient(qd2, qd1),
            FullGaussian.from_quotient(q2, q1),
        )
...
def test_damped_updates() -> None:
    ...
    for _ in range(7):
        ...
        q = FullGaussian(eta=get_random_eta(), Lambda=get_random_Lambda())
        qs = [
            FullGaussian(eta=get_random_eta(), Lambda=get_random_Lambda())
            for _ in range(11)
        ]
        q1 = q.damped_updates(qs, delta=delta)
        q2 = FullGaussian(
            eta=q.eta + delta * FullGaussian.from_products(qs).eta,
            Lambda=q.Lambda + delta * FullGaussian.from_products(qs).Lambda,
        )
