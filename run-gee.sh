...
for algorithm in $algorithms; do
    ...
    if [ "$mode" = "gold" ]; then
        ...
        case "$algorithm" in
            "ep")
                algo_params="algorithm=ep algorithm.ep.n_points=10"
                ;;
            "hmc")
                algo_params="algorithm=hmc"
                ;;
            "rw")
                algo_params="algorithm=rw algorithm.rw.sd_shrink_factor=0.3 algorithm.rw.n_samples=4000000 algorithm.rw.thinning=40"
                ;;
            "vi")
